...
int write_cla()
{
    FILE *fp_w;
    int i;
	if((fp_w = fopen("./moduleB/class_i.txt","w")) == NULL)
	{
		printf("File error!\n");
		exit(0);
	}
...
{
	int i=0,j,num=0,n=0;
    printf("please input the students' courses number:");
    scanf("%d",&n);
    printf("please input course information NUMBER LESSON KIND TIME POINT STUDENT TOTAL:\n");
	for(i=0;i<n;i++)
	{
		scanf("%d %s %c %d %d %d %d",&cla[i].bh,cla[i].mc,&cla[i].xz,&cla[i].xs,
			&cla[i].xf,&cla[i].yx_num,&cla[i].sx_num);
        for(j=0;j<i;j++)
            if(cla[i].bh == cla[j].bh || strcmp(cla[i].mc,cla[j].mc)==0)
            {
...
int add_cla()
{
    ...
    log_cla("Add",cla[i]);
	FILE *fp_a;
	if((fp_a = fopen("./moduleB/class_i.txt","a")) == NULL)
	{
		printf("File error!\n");
		exit(0);
	}
